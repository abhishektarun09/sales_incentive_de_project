...
def sales_mart_calculation_table_write(final_sales_team_data_mart_df):
    ...
    rank_window = Window.partitionBy("store_id", "sales_month")\
                                    .orderBy(col("total_sales_each_month").desc())
    final_sales_team_data_mart_table = final_sales_team_data_mart\
                                    .withColumn("rnk", rank().over(rank_window))\
                                    .withColumn("incentive",  when(col("rnk") == 1, col("total_sales_each_month") * 0.01)
                                                .otherwise(lit(0)))\
                                    .withColumn("incentive", round(col("incentive"), 2))\
                                    .withColumn("total_sales", col("total_sales_each_month"))\
                                    .select("store_id", "sales_person_id", "full_name", "sales_month", "total_sales",
                                            "incentive")
